"""Study 03: original compound sheet-metal geometry, with exact native profiles.
...
ROOT=Path.cwd()/'.local'/'sheet-metal-study'/'complex'
...
def beads(r,base,centers,straight,top,H,t,rm,z=0):
    steps=[S(top),B(rm,-90),S(H-2*rm),B(rm,90),S(3*t)]
    edges,end=a.strip((0,z+H+t/2),(1,0),steps,t);rend=end[0];root=top+2*rm
    master=a.revolve(r,'Closed bead master',edges);X,Y,Z=coords(r);cuts=[];patch=[]
    for i,(cx,cy) in enumerate(centers):
        ...
        patch.append(a.mapped(r,f'Closed bead {i}',master,rho,Z,[cx-straight/2-rend,cy-rend,z],[cx+straight/2+rend,cy+rend,z+H+t]))
...
def finish(r,body,volume,t,ri,outside,holes,summary,source):
    ...
    return r,body,a.base_meta(t,ri,volume,outside_mm=outside,through_openings=holes,summary=summary,source_id=source,developed_blank=False,motion=r.motion)
...
def vent_chassis():
    ...
    half=[S(floor/2),B(rm,90),S(H-2*rm),B(rm,-90),S(flange),B(rm,-90),S(ret)]
    ...
    dh=r.param('Mount hole diameter',5);body,dv=holes(r,body,[(L*.38*s,(floor/2+2*rm+flange/2)*sy) for s in [-1,1] for sy in [-1,1]],dh,t,H);volume-=dv
    return finish(r,body,volume,t,ri,[v(L),2*v(end[0])+v(t),v(H+t)],10,'Six perimeter bends combine mounting ledges, downturned returns and six 30-degree lanced vents.','protolabs-forms')
...
def beaded_crossmember():
    ...
    half=[S(crown/2),B(rm,-90),S(H-2*rm),B(rm,90),S(foot),B(rm,45),S(leg),B(rm,-45),S(tip)]
...
def stepped_housing():
    ...
    steps=[S(fr),B(rm,90),S(h1-2*rm),B(rm,-90),S(shelf),B(rm,90),S(h2-2*rm),B(rm,-90),S(flange)]
...
def annular_cover():
    ...
    H=drop+lower;start=(rh,H+rm+neck+t/2);leg=(drop-2*rm*(1-math.cos(math.pi/4)))/math.sin(math.pi/4)
    steps=[S(neck),B(rm,90),S(deck),B(rm,-45),S(leg),B(rm,45),S(shelf),B(rm,-90),S(lower-2*rm),B(rm,90),S(flange)]
    edges,end=a.strip(start,(0,-1),steps,t);body=a.revolve(r,'Collared cover stock',edges);_,moment=a.section_integrals(edges);volume=2*math.pi*moment
    r.motion={'kind':'round','chain':anim_chain(start,(0,-1),steps,t),'features':[]}
    for i,(p,u,kind) in enumerate(profile_samples(start,(0,-1),steps)):
        for d,label,ex in [(-v(t)/2,'face A','zero'),(0,'stock','negative'),(v(t)/2,'face B','zero')]:
    ...
    dh2=r.param('Upper deck hole diameter',4);pr=rh+rm+deck/2
    ...
    return finish(r,body,volume,t,ri,[2*v(end[0]),2*v(end[0]),v(start[1])],13,'An integral upright neck joins two stepped annular decks, a 45-degree cone, and a pierced outer flange.','mate-forms')
...
def cable_panel():
    ...
    L=r.param('Length',200);W=r.param('Outside width',140);H=r.param('Edge flange height',15);floor=W/2-ro
    half=[S(floor),B(rm,90),S(H-ro)]
