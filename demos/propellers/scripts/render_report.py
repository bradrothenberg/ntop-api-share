...
'''
    ...
    for item in manifest['cases']:
        ...
        family={'airfoil':'Airfoil candidate','elliptical':'Original section study','marine':'Marine loop screw'}[item['group']]
...
<div class="note"><strong>Recorded geometry, ready for native replay.</strong><p>Twenty-three complete construction graphs cover the eight-inch assembly, separate spinner, twenty aircraft studies, and the fuller marine screw with a service cap. The two assembly notebooks retain their native caches. Large cached blade notebooks and STL files are generated locally by replay.</p><p>Source models were evaluated with nTop build 42926. Public packaging and replay checks run offline. No accepted aerodynamic optimum, acoustic prediction, or safe-speed qualification is claimed.</p></div>
...
<h2><span class="num">02</span> Inspect the blade studies</h2>
...
<div class="filters" role="group" aria-label="Blade family"><button data-filter="all" aria-pressed="true">All designs</button><button data-filter="airfoil" aria-pressed="false">Airfoil candidates</button><button data-filter="elliptical" aria-pressed="false">Original studies</button><button data-filter="marine" aria-pressed="false">Marine screw</button></div><div class="grid">{''.join(cards)}</div>
<h2><span class="num">03</span> Marine proportions and service cap</h2><p>The nominal 3.048 m marine screw uses three uninterrupted return loops. Broad roots and a 0.30 barrel-to-diameter ratio follow the civilian product-photo reference. These are authored proportions, not recovered proprietary dimensions.</p><div class="grid"><figure><img src="assets/marine_prop_front.png" alt="Native nTop aft view with separate cap and eight screws"><figcaption>The aft service cap has eight M24 screws on a 762 mm bolt circle.</figcaption></figure><figure><img src="assets/marine_prop_forward.png" alt="Native nTop forward view of the open barrel"><figcaption>The open barrel still needs a shaft-specific spline cartridge and an axial propeller retainer.</figcaption></figure></div><p><a href="../MARINE.md">Marine construction, cap dimensions, limits, and sources</a>. Cap fasteners retain the cover only. Hydrodynamic efficiency and structural strength remain unmeasured.</p><h2><span class="num">04</span> Reproduce and inspect</h2>
...
<h2><span class="num">05</span> Flow optimization and remaining work</h2>
<p>A rotating FUN3D smoke test completed, and the wall velocities matched the prescribed rotation. The longer trial reached 956 iterations before its runtime cutoff. A later boundary-layer mesh passed positive-cell-volume checks. Wall resolution and grid sensitivity, converged performance, adjoint gradient checks, unsteady wake output, and acoustic prediction remain open.</p>
