...
- Use `uv run --locked python scripts/build.py fcat` from the repository root to replay and check the graph.
- Use `uv run --locked python scripts/stage.py fcat` to prepare an import for an empty nTop notebook.
